...
setwd("~/Documents/0_MASTERS/Programming/ps9-nhoang85")
...
ipip.l <- ipip %>% 
  gather(key= "Big5", value = "trait_values", 7:48)
...
ipip.l <- ipip.l %>% 
  separate(Big5, into = c('trait','item'), sep = "_")

# Calculate averages for each participant (coded as RID) and trait:
ipip.comp <- ipip.l %>% 
  group_by(RID,trait) %>% 
  summarise(trait_avg=mean(trait_values))
...
ipip.comp <- ipip %>% 
  select(one_of('RID','age','gender','exer','BMI')) %>% 
  merge(ipip.comp, by.x = 'RID',all = TRUE)
...
exer.avg <- ipip.comp %>% 
  group_by(trait,exer) %>% 
  summarise(n=length(RID),avg=mean(trait_avg),sem=sd(trait_avg)/sqrt(n))
